...
def alarm_allowed_on_datetime(alarm: Alarm, some_datetime: datetime) -> bool:
    ...
    return (
        not alarm.repeat or weekday in alarm.repeat
    ) and some_datetime > datetime.now()


def get_alarm_next_ring_time(alarm: Alarm) -> datetime:
    next_datetime = datetime.combine(date=datetime.today(), time=alarm.time)
...
def get_next_alarm(alarms: Sequence[Alarm]):
    try:
        return min(
            (alarm for alarm in alarms if alarm.enabled), key=get_alarm_next_ring_time
        )
    except ValueError:
        return None
